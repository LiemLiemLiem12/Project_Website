...
class AdminSettingController {
    ...
    public function update() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Lấy tất cả các settings hiện tại để biết loại của từng setting
            $allSettings = $this->settingModel->getAllSettings();
            $settingTypes = [];
            foreach ($allSettings as $setting) {
                $settingTypes[$setting['setting_key']] = $setting['setting_type'];
            }
            ...
            // Xử lý các boolean settings không được gửi (unchecked)
            foreach ($settingTypes as $key => $type) {
                if ($type === 'boolean') {
                    $postKey = 'setting_' . $key;
                    if (!isset($_POST[$postKey])) {
                        // Nếu là boolean setting và không có trong POST data, set giá trị là 0
                        $this->settingModel->updateSetting($key, '0');
                    }
                }
            }
