    ...
    cartItems.forEach(item => {
        ...
        if (priceElement) {
            ...
            if (priceElement.querySelector('div')) {
                price = parseInt(priceElement.querySelector('div').textContent.replace(/\D/g, ''));
            } else {
            ...
            console.log(`Item ${item.dataset.id}: Unit price initialized to ${price}`);
        }
    });
}
...
function applyVoucherCode(code) {
    ...
    switch(code) {
        case 'SALE10':
            ...
            updateCartNotification(`Đơn hàng của bạn đã được giảm 10% nhờ nhập mã <strong>${code}</strong>`);
            break;
        ...
        case 'SALE30K':
            ...
            if (subtotal >= 599000) {
                ...
                updateCartNotification(`Đơn hàng của bạn đã được giảm 30K nhờ nhập mã <strong>${code}</strong>`);
            } else {
                showCouponMessage(`Mã giảm giá chỉ áp dụng cho đơn hàng từ 599K!`, 'error');
            }
            break;
        ...
        case 'SALE70K':
            ...
            if (subtotal2 >= 899000) {
                ...
                updateCartNotification(`Đơn hàng của bạn đã được giảm 70K nhờ nhập mã <strong>${code}</strong>`);
            } else {
                showCouponMessage(`Mã giảm giá chỉ áp dụng cho đơn hàng từ 899K!`, 'error');
            }
            break;
        ...
        case 'SALE100K':
            ...
            if (subtotal3 >= 1199000) {
                ...
                updateCartNotification(`Đơn hàng của bạn đã được giảm 100K nhờ nhập mã <strong>${code}</strong>`);
            } else {
                showCouponMessage(`Mã giảm giá chỉ áp dụng cho đơn hàng từ 1,199K!`, 'error');
            }
            break;
        ...
        case 'FREESHIP':
            ...
            updateCartNotification(`Đơn hàng của bạn đã được miễn phí vận chuyển nhờ nhập mã <strong>${code}</strong>`);
            break;
            
        case 'SALE50K':
            showCouponMessage('Mã giảm giá 50K đã được áp dụng!', 'success');
            updateCartTotal(0, false, 50000); // Giảm 50K
            updateCartNotification(`Đơn hàng của bạn đã được giảm 50K nhờ nhập mã <strong>${code}</strong>`);
            break;
...
function updateItemPrice(item) {
    ...
    console.log(`Item ${item.dataset.id}: Updated price - ${quantity} x ${unitPrice} = ${totalPrice}`);
}

function updateCartTotal(discountPercent = 0, freeShipping = false, fixedDiscount = 0) {
    // Calculate subtotal
    let subtotal = 0;
    const cartItems = document.querySelectorAll('.cart-item');
    ...
    console.log(`Cart updated: Subtotal=${subtotal}, Discount=${discount}, Shipping=${shipping}, Total=${total}`);
...
function updateShippingProgress(subtotal) {
    ...
    if (progressBar && progressText) {
        ...
        if (subtotal >= freeShippingThreshold) {
            ...
            progressText.textContent = 'Còn ' + formatCurrency(remaining) + ' để được miễn phí vận chuyển';
        }
    }
}
